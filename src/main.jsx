...
function TechnicalCardsPage({ account }) {
  ...
  return <div className="technical-cards-page">
    ...
    {selected&&<div className="technical-preview-modal" role="dialog" aria-modal="true" aria-label={`${selected.title} ön izlemesi`} onMouseDown={(event)=>{if(event.target===event.currentTarget)setSelected(null)}}><section><header><span><small>TEKNİK KART ÖN İZLEMESİ</small><h2>{selected.title}</h2><p>{selected.category} · Kart {String(selected.number).padStart(2,"0")}</p></span><button type="button" onClick={()=>setSelected(null)} aria-label="Ön izlemeyi kapat"><X/></button></header><div className="technical-modal-document"><img key={selected.id} src={`https://drive.google.com/thumbnail?id=${selected.id}&sz=w1600`} alt={`${selected.title} teknik kart ön izlemesi`}/></div><footer><a href={`${docUrl(selected.id,"export")}?format=docx`}><Download/> Word İndir</a><a href={`${docUrl(selected.id,"export")}?format=pdf`}><Download/> PDF İndir</a><button type="button" onClick={()=>shareCard(selected)}><MessageCircle/> WhatsApp ile Paylaş</button></footer></section></div>}
  </div>;
}
function Empty() {
  return (
