...
export default function Ready365LibraryPage({account,go}){
  ...
  const triggerRef=useRef(null),closeRef=useRef(null);
  const closeModal=()=>{setActive(null);requestAnimationFrame(()=>triggerRef.current?.focus())};
  const openModal=(item,event)=>{triggerRef.current=event.currentTarget;setActive(item)};
  useEffect(()=>{
    if(!active)return;
  ...
  return <div className="page ready-library-page">
    ...
    {active&&<div className="ready-modal" role="dialog" aria-modal="true" aria-label={`${active.title} PDF görüntüleyici`} onMouseDown={(event)=>{if(event.target===event.currentTarget)closeModal()}}><div className="ready-modal-panel"><header><span><small>{active.category}</small><h2>{active.title}</h2><p>{active.original}</p></span><button ref={closeRef} onClick={closeModal} aria-label="PDF görüntüleyiciyi kapat"><X/></button></header><div className="ready-pdf-frame"><iframe src={active.preview} title={`${active.title} PDF`} allow="autoplay" allowFullScreen/></div></div></div>}
  </div>;
}
